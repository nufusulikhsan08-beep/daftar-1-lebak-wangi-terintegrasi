...
class StoreTeacherRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        if ($this->employment_status === 'ASN') {
            $rules['nip'] = 'required|string|max:20|unique:teachers,nip,' . $this->route('teacherId');
            $rules['tmt_cpns'] = 'required|date';
            $rules['tmt_pns'] = 'required|date|after_or_equal:tmt_cpns';
        } else {
            $rules['nip'] = 'nullable|string|max:20|unique:teachers,nip,' . $this->route('teacherId');
        }

        // NUPTK optional but unique if provided
        $rules['nuptk'] = 'nullable|string|max:20|unique:teachers,nuptk,' . $this->route('teacherId');
